...
class LandcoverML:
    ...
    def run_classifier(self, pixel_based_only=False, regression = False, label_column = 'code_lu'):
        ...
        training_pixel = training_pixel.filter(ee.Filter.notNull(self.input_image.bandNames().getInfo() + [label_column]))
        validation_pixel = validation_pixel.filter(ee.Filter.notNull(self.input_image.bandNames().getInfo() + [label_column]))
        ...
        if pixel_based_only == False:
            ...
            training_stats_segmented = training_stats_segmented.filter(ee.Filter.notNull(self.cluster_properties.bandNames().getInfo() + [label_column]))
            validation_stat_segmented = validation_stat_segmented.filter(ee.Filter.notNull(self.cluster_properties.bandNames().getInfo() + [label_column]))
